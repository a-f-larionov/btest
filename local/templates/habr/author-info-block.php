    ...
    <a href="http://test/bitrix/admin/user_edit.php?lang=ru&ID=<?= $AUTHOR['ID'] ?>"
       ...
       title="Автор публикации">
        ...
        <? if ($AUTHOR['PERSONAL_PHOTO']) { ?>
            ...
            <img src="//habrastorage.org/getpro/habr/avatars/877/493/d58/877493d580304894f45ddf1a4844de04.jpg"
                 width="24" height="24"
        ...
        <? } ?>
        <span class="user-info__nickname user-info__nickname_small"><?= $AUTHOR['LOGIN'] ?></span>
    </a>


    <span class="post__time">
                        <? if (date("Y-m-d", $ITEM['DATE_CREATE_UNIX'])
                            == date("Y-m-d", time())
                        ) {
                            echo 'сегодня';
                        } elseif (date("Y-m-d", $ITEM['DATE_CREATE_UNIX'])
                            == date("Y-m-d", time() - 86400)
                        ) {
                            echo 'вчера';
                        } else {
                            echo date('Y-m-d', $ITEM['DATE_CREATE_UNIX']);
                        } ?>
        в
        <?= date('H:i', $ITEM['DATE_CREATE_UNIX']) ?>
                        </span>
